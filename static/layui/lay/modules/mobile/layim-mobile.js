...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var comTpl = function(tpl, anim, back){
    return ['<div class="layim-panel'+ (anim ? ' layui-m-anim-left' : '') +'">'
      ,'<div class="layim-title" style="background-color: {{d.base.chatTitleColor}};">'
        ,'<p>'
          ,'{{# if(d.base.back){ }}'
            ,'<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>' 
          ,'{{# } }}'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'{{# if(d.base && d.base.channels){ }}'
        ,'<div class="layui-row">'
          ,'{{# layui.each(d.base.channels, function(index, item){ }}'
            ,'<button class="layui-btn" layim-event="pay_channel" lay-filter="{{item.name}}" lay-index="{{index}}">{{item.name}}</button><span> </span>'
          ,'{{# }); }}'
        ,'</div>'
      ,'{{# }; }}'
      ,'<div class="layim-chat-send"><input type="text" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">发送</button><button class="layim-send" layim-event="finish">充好了</button></div>'
  ...
  var elemChatMain2 = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>'
      ,'{{ d.username||"佚名" }}'
    ,'</cite></div>'
    ,'<div class="pay_channel_container">'
      ,'<div class="pay_channel"s>'
          ,'<a href="/payinfo.html?type={{ d.pay_type }}&qrcode_url={{ d.qrcode_url }}&param1={{ d.params[0] }}&param2={{ d.params[1] }}&param3={{ d.params[2] }}&param4={{ d.params[3] }}" target="_blank">'
            ,'<img src={{ d.pay_icon }} class="pay_channel_icon" align="left" hspace="5">'
            ,'<hr class="top_line">'
            ,'<div class="pay_channel_title">{{ layui.data.content(d.content||"&nbsp;") }}<br><span class="pay_channel_content">点击开始充值</span></div>'
            ,'<hr/>'
            ,'<div class="pay_channel_bottom_text">收款</div>'
          ,'</a>'
      ,'</div>'
    ,'</div>'
  ,'</li>'].join('');
  ...
  //发送消息
  var sendMessage = function(initData, pay_type){
    var data = {
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if(data.system){
      ...
      if (data.pay) {
        ul.append(laytpl(elemChatMain2).render(data));
      }else {
        ul.append(laytpl(elemChatMain).render(data));
      }
    }
  ...
  var viewChatlog = function(){
    ...
    layui.each(chatlog[thatChat.data.type + thatChat.data.id], function(index, item){
      ...
      if (item.pay) {
        ul.append(laytpl(elemChatMain2).render(item));
      }else {
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    // 选择支付方式
    ,pay_channel: function(othis){
      var name = othis.attr('lay-filter');
      var index = othis.attr('lay-index');
      
      var message = '用' + name + '支付';
      sendMessage(message, parseInt(index));
    }
