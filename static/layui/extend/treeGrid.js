...
layui.config({
    ...
    var $ = layui.$
        ...
        ,table = {
            ...
            ,treeNodeOpen:function (o, isOpen) {
                var that=table.getClass(tableId);
                if(!that||!o)return;
            ...
            ,treeOpenAll:function (tableId,isOpen) {
                ...
                list.forEach(function (temo) {
                    table.treeNodeOpen(temo,isOpen);
                });
            }
            /**
             * 获取全部需要子节点对象集合
     ...
     * icon渲染
     * @param o
     * @param isHtml  true  false   是否返回html
     */
        ...
        that.elem.on('click','input[name="layTableCheckbox"]+', function(){
            ...
            if(isAll){
                ...
                if(options.isTree){
                    ...
                    //处理上级
                    var uo=that.treeFindUpData(obj);
                    if(uo&&uo.children.length>0){
                        var temis=true;
                        uo.children.forEach(function (temo) {
                            if(!temo[table.config.cols.isCheckName]){
                                temis=false;
                            }
                        });
                        if(temis){
                            that.setCheckData(uo[table.config.indexName], checked);
                        }
                    }
                }
                that.syncCheckAll();
